...
#+END_QUOTE
The encrypted amount is defined to be \(a \oplus H( \text{"amount"} , H(r V_i , t)) \) where \( r \) seems to be the private transaction key and \( V_i \) the payee's viewkey. Since \( r V_i =  r v_i g = v_i r g = v_i R \).
*
*