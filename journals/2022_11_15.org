...
Let \( \{ s_i \} \), \( \{ t_i \} \) be two subsets of \( \mathbb{F} \) which have equal non-zero length. Then \( \prod_i (s_i - \beta) = \prod_i (t_i - \beta) \) has negligible  probability if \( \{ s_i \} \) is not a permutation of \( \{ t_i \} \). This is an application of