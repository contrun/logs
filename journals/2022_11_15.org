...
However, if overlapping subgroups are used for both domains, then \( \omega^{1/\rho} = \omicron \) and \() generates the larger domain whereas ο generates the smaller one.
#+END_QUOTE