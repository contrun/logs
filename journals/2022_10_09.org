...
copy-satisfy is the primitive needed in the plonk protocol. It means everything within the partition has the same value. In terms of circuits, it means the values of gates belonging to the same partition are equal (like ).
