...
+ Commit: evaluate \( f \) at \( S \), send the Merkle tree root \( r \) of the result of the evaluation.
+ Open: for a value \( z \), let \( y = f(z) \), evaluate \( g \) at \( S \), send the Merkle tree root \( r \) of the result of the evaluation, send the proof that the polynomial \( g = \frac{f(X) - y}{X - z} \) is of degree \(\mathrm{deg}(f) - 1 \), commit the polynomial \( g \)
