...
Here the result is calculated by ~ df['result'] = df.category0 if df.tgt == 0 else df.category1 ~. The cross entropy loss is calculated as \( \frac{}\sum -\log(\mathrm{result}) \). The question is that why is it called cross entropy?
*