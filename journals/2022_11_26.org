...
To encrypt a message \( M \in \{0,1\}^n \) to under the identification \( \text{id} \), let \( r \in 
 \Z_q^*\) be a random scalar, output \( C = (rP, H(e(r Q_{\text{id}}, P_{\text{pub}})) \oplus M) \).
** Decrypt (without chosen cipher-text security)
Let \( (U, V) \) be the cipher text, noting that \( e(r Q_{\text{id}}, P_{\text{pub}}) = e(r Q_{\text{id}}, s P) = e(s Q_{\text{id}}, r P) = e(s_{\text{id}}, rP) \), we can see \( H(e(s_{\text{id}}, U)) \oplus V \) is the clear text.
...
To encrypt a message \( M \in \{0,1\}^n \) to under the identification \( \text{id} \), choosing a random \( \sigma \in \{0, 1\}^n \), let \( r = H_3(\sigma, M) \in 
 \Z_q^*\), output \( C = (rP, \sigma \oplus H(e(r Q_{\text{id}}, P_{\text{pub}})) \oplus M) \).
