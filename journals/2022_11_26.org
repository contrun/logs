...
On one hand, reading/writing memory is significantly slower than the running of AES encryption/decryption instruction. On the other hand, changing data bytes by bytes (encryption/decryption) will obviously have a high overhead on the reading/writing of data. So the corollary is that AES encryption/decryption must work a bulk of data (maybe it takes memory addresses as arguments, does not).
*