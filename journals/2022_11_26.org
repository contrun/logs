...
Since the same physical memory should never be mapped twice, when the guest claims some physical memory, the hypervisor may not reuse this physical memory anymore. What if the guest continues to hold this memory, and refuse to give up? There must be some clean up mecha