...
Let \( e: G_1 \times G_1 \to G_2 \) be a bilinear non-degenerate Elliptic curve pairing. Let \( H_1: \{0, 1, 2\}^* \to G_1^* \) be a hash function from strings to the elliptic group \( G_1^*\), \( H_2: G_2 \to \{1, 2\}^n \) be a hash function for some \( n \in \Z \), \( H_3: \{1, 2\}^n \times \{1, 2\}^n \to \Z_q^* \) be a hash function, \( H_4: \{1, 2\}^n \to \{1, 2\}^n \) be a hash function.
** Setup
...
** Extract
Given a identification \( \text{id} \in \{1,2\}^* \), computes \( Q_{\text{id}} = H_1(\text{id}) \in G_1^* \) as the public key of \(text{id}\), set the private key of \( s_{\text{id}} = sQ_{\text{id}} \).
** Encrypt
To encrypt a message \( M \in \{0,1\}^n \) to under the identification \( \text{id} \)
** Decrypt