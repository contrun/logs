...
Say ~son_os~ is a guest os managed by the os ~os~, ~grandson_os~ is a guest os managed by ~son_os~. We can translate ~grandson_os~'s virtual memory address ~vma~ to the fake physical memory address ~fpma~