...
On one hand, reading/writing memory is significantly slower than the running of AES encryption/decryption instruction. On the other hand, changing data bytes by bytes () will obviously have a high overhead on the
*