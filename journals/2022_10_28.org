...
In a quick glance, type classes, traits and interfaces both require the implementor to implement some methods. What are the difference between these concepts? For one thing, type classes itself are not types. We can say the implementor of traits/interfaces are subclass of tr